...
with open('./data-union/M-L-D_network.txt', 'w') as f:
    for item in adj_list:
        f.write('\t'.join([str(x) for x in item]) + '\n')

print("Deepwalk training...")
os.system("deepwalk --input " + './data-union/M-L-D_network.txt '
          + "--number-walks 80 --representation-size 128 "
          + "--walk-length 40 --window-size 10 --workers 8 --output " + './data-union/embeddings.txt')
